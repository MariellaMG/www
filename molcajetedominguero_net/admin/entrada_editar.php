...
?>
   ...
   <?php
	...
	while ($rowt = mysql_fetch_row($resultt)){
		...
		$titulo = utf8_encode($rowt[2]);
		$descripcion = utf8_encode($rowt[3]);
		$megusta = $rowt[4];
		$comentarios = $rowt[5];
		$fecha = $rowt[6];
		$imagen = $rowt[7];
...
<h1><img src="img/icons/posts.png" alt="" />Editar '<?php echo $titulo; ?>'</h1>
...
<form id="form1" method="post" enctype="multipart/form-data" action="entrada_agregar_form.php">
	...
	while ($row = mysql_fetch_row($resultt)){
		...
		echo '<option value="'.$row[0].'" '.$sel.' >'.utf8_decode($row[1]).'</option>
		';
	}
	?>
            </select>
...
<div class="input medium" >
     ...
     <input name="archivos" id="archivos" type="hidden" value="<?php echo $fotos; ?>">
