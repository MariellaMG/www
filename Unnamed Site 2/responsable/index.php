...
function Suspendida(t_id){
	if(confirm("Estas seguro que es una cuenta SUSPENDIDA?")){
		$.get("suspendida.php?id="+t_id, function(data){
			//alert(data);
			//location.reload();
			$("#linea"+t_id).hide();
		});
...
function SalvarPass(t_id){
	var pass=$("#pass"+t_id).val();
	$.get("salvar_pass.php?id="+t_id+"&pass="+pass, function(data){
		//alert(data);
	});
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  $qt = "SELECT * FROM revisar WHERE id_responsable='$id_capturista' AND suspendia=0 ";
	//echo $qt;
	$resul = mysqli_query($link,$qt);
	while ($rowt = mysqli_fetch_row($resul)){
		$id_revisar = $rowt[0];
  ...
  ?>
   ...
   <tr style="height:40px;" id="linea<?php echo $id_revisar; ?>">
   <a name="linea<?php echo $id_revisar; ?>" ></a>
    <td><input type="text" style="width:80%;" value="<?php echo $nombre ?>" /></td>
    
    <td><input type="text" style="width:60%;" value="<?php echo $usuario ?>" />
    </td>
    
    <td><input type="text" style="width:60%;" value="<?php echo $pass ?>" id="pass<?php echo $id_revisar; ?>" />
    <input name="salvarusuario" type="button" value="SAVE" onclick="SalvarPass('<?php echo $id_revisar; ?>');" />
    </td>
    <td><input type="text" style="width:80%;" value="<?php echo $mail ?>" /></td>
    <td><input type="text" style="width:80%;" value="<?php echo $mail_pass; ?>" /></td>
    <td><a href="#linea<?php echo $id_revisar; ?>" onclick="Suspendida(<?php echo $id_revisar; ?>);">SUSPENDER <?php echo $usuario; ?></a></td>
  </tr>
