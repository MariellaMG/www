...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  $qt = "SELECT * FROM revisar WHERE supervicion_api='1' ORDER BY conteo  ";
	//echo $qt;
	$resul = mysqli_query($link,$qt);
	while ($rowt = mysqli_fetch_row($resul)){
		$id_revisar = $rowt[0];
		...
		$id_responsable = $rowt[14];
		
		$conteo = $rowt[15];
  
  		$qt = "SELECT * FROM capturista WHERE id_capturista = '$id_responsable' ";
		//echo $qt;
  ...
  ?>
   ...
   <tr style="height:40px;">
    ...
    <td><input type="text" style="width:80%;" value="<?php echo $mail_pass; ?>" /></td>
     <td><input type="text" style="width:80%;" value="<?php echo $conteo; ?>" /></td>
     
      <td><input type="text" style="width:80%;" value="<?php echo $nombre; ?>" /></td>
     
     <td><A href="http://tinkeringdesigns.com/responsable/?usr=<?php echo $usuario; ?>">LIGA</a></td>
