...
$salida = $twitter->setGetfield($getfield)
	   ...
	   if(is_array($value)){ 
			...
			if($key=="text"){
				if(strlen($value)>22){
					...
					$qt = "SELECT texto FROM tweet_aleatorio WHERE texto LIKE '%$mini%' LIMIT 1";
					//echo $qt;
					$resul = mysqli_query($link,$qt);
					$cc=0;
					while ($rowt = mysqli_fetch_row($resul)){
					...
					$qt="INSERT INTO `tweet_aleatorio` (`id_tweet_aleatorio`, `texto`, `extra`, `estatus`) VALUES (NULL, '$value', '', '');";
					//echo $qt."<br />";
					$resul = mysqli_query($link,$qt);
					}
	...
	$qt = "DELETE FROM `tweet_aleatorio` WHERE `tweet_aleatorio`.`texto` LIKE '%/%' ";
	//echo $qt;
	$resul = mysqli_query($link,$qt);
	
	$qt = "DELETE FROM `tweet_aleatorio` WHERE `tweet_aleatorio`.`texto` LIKE '%http%' ";
	//echo $qt;
	$resul = mysqli_query($link,$qt);
