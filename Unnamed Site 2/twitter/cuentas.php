...
*/
	...
	$qt = "SELECT * FROM revisar WHERE revisada = '1' AND suspendia=0 ORDER BY supervizada LIMIT 1";
	//echo $qt;
	$resul = mysqli_query($link,$qt);
	while ($rowt = mysqli_fetch_row($resul)){
		$id_revisar = $rowt[0];
	...
	$qt = "UPDATE `revisar` SET `supervizada` = supervizada+1 WHERE `revisar`.`id_revisar` = '$id_revisar';";
	$resul = mysqli_query($link,$qt);
...
<?php
$qt = "SELECT count(*) FROM revisar WHERE supervizada = '1' AND suspendia=0 ";
	//echo $qt;
	$resul = mysqli_query($link,$qt);
	while ($rowt = mysqli_fetch_row($resul)){
		$revizadas = $rowt[0];
...
<h1>Supervizada: <?php echo $revizadas; ?></h1>
