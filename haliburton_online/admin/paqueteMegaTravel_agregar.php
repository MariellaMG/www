...
<?php
	...
	while ($rowt = mysql_fetch_row($resultt)){
		...
		$id_contenido = $rowt[0];
		$url = $rowt[1];
		$completo = utf8_encode($rowt[2]);
		$titulo = utf8_encode($rowt[3]);
		$codigo = $rowt[4];
		$nombre = utf8_encode($rowt[5]);
		$ciudades = utf8_encode($rowt[6]);
		$imagen = $rowt[7];
		$duracion = utf8_encode($rowt[8]);
		$precio = $rowt[9];
		$fecha = $rowt[10];
		$vigencia = $rowt[11];
		$avion = $rowt[12];
	...
	$qt = "SELECT * FROM zona WHERE id_zona='".$id_zona."' ";
	$resultt = mysql_query($qt, $db);
	while ($rowt = mysql_fetch_row($resultt)){
		//echo utf8_encode($rowt[1]);
		$zona = utf8_encode($rowt[1]);
	}
?>
...
<h1><img src="img/icons/posts.png" alt="" />Agregar paquete Mega Travel '<?php echo $completo; ?>'</h1>
...
<form id="form1" method="post" enctype="multipart/form-data" action="paquete_agregar_form.php">
    ...
    <input name="id_contenido" id="id_contenido" type="hidden" value="<?php echo $id_contenido; ?>">
...
<input name="silueta_img" id="silueta_img" type="hidden" value="">
    ...
    <div class="content">
        ...
        <div class="input medium">
            ...
            <input type="text" id="titulo" name="titulo" value="<?php echo $nombre; ?>" />
        ...
        <div class="input medium">
            ...
            <input type="text" id="region" name="region"  value="<?php echo $zona; ?>" />
            <span>Ingresa la región o país del viaje</span>
        </div>
        
        
        <div class="input medium" style="width:100%;">

            <label for="input2">Ciudad</label>
            <input type="text" id="ciudad" name="ciudad"  value="<?php echo $ciudades; ?>" size="150" style="width:100% !important;" />
    ...
    <?php
	$qt = "SELECT * FROM zona ORDER BY nombre ";
	$resultt = mysql_query($qt, $db);
	...
	?>
        ...
        <div class="input medium" style="height:420px;">

            <label for="input2">HTML</label>
            <div style="background:#FFF; border:solid 2px #666666;">
            <iframe src="paqueteMegaTravel_html.php?id=<?php echo $id_contenido; ?>" width="100%" height="400">
            </iframe>
            </div>
        </div>
